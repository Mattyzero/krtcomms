...
const char* ts3plugin_description() {
	...
	static char* result = NULL;  /* Static variable so it's allocated only once */
	if (!result) {
		const wchar_t* name = L"Für bessere Kommunikation zwischen Anführern";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Fuer bessere Kommunikation zwischen Anfuehrern";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "Fuer bessere Kommunikation zwischen Anfuehrern";
#endif
}
...
void ts3plugin_initHotkeys(struct PluginHotkey*** hotkeys) {
	...
	CREATE_HOTKEY("send_ch_1_", "Radio 2 END");
...
int ts3plugin_onTextMessageEvent(uint64 serverConnectionHandlerID, anyID targetMode, anyID toID, anyID fromID, const char* fromName, const char* fromUniqueIdentifier, const char* message, int ffIgnored) {
	...
	//ts3Functions.printMessageToCurrentTab(QString::number(fromID).toStdString().c_str());
...
void ts3plugin_onEditCapturedVoiceDataEvent(uint64 serverConnectionHandlerID, short* samples, int sampleCount, int channels, int* edited) {
	KRTComms::getInstance().OnEditCapturedVoiceDataEvent(serverConnectionHandlerID, samples, sampleCount, channels, edited);
}
...
void ts3plugin_onPluginCommandEvent(uint64 serverConnectionHandlerID, const char* pluginName, const char* pluginCommand, anyID invokerClientID, const char* invokerName, const char* invokerUniqueIdentity) {
	...
	//QString logmessage = "onPluginCommandEvent___ serverConnectionHandlerID:" + QString::number(serverConnectionHandlerID) +
	//	" | pluginName: " + QString(pluginName) + " | pluginCommand: " + QString(pluginCommand) + " | invokerClientID: " + QString::number(invokerClientID) + " | invokerName: " + QString(invokerName) +
	//	" | invokerUniqueIdentity: " + QString(invokerUniqueIdentity);
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	...
	char url[256] = "https://robertsspaceindustries.com/citizens/";
	...
	switch(type) {
		...
		case PLUGIN_MENU_TYPE_CLIENT:
			...
			if (ts3Functions.getClientVariableAsString(serverConnectionHandlerID, (anyID)selectedItemID, CLIENT_NICKNAME, &name) != ERROR_ok) {
				printf("Error getting client nickname\n");
				return;
			}

			switch(menuItemID) {
				case MENU_ID_CLIENT_CID:
					
					
					citizen = strtok_s(name, " ", &nextToken);
					
					if (citizen != NULL) {
						strcat_s(url, citizen);
						//Muss vor dem urlsToBB sein LUL
						
						ShellExecuteA(NULL, NULL, url, NULL, NULL, SW_SHOWNORMAL);

						ts3Functions.urlsToBB(url, url, 256);
...
void ts3plugin_onHotkeyEvent(const char* keyword) {
	...
	//ts3Functions.printMessageToCurrentTab(keyword);

	uint64 serverConnectionHandlerID = ts3Functions.getCurrentServerConnectionHandlerID();

	QString keyword_(keyword);
	if (keyword_.startsWith("send_ch_")) {
		bool ok;
...
void channelsui_show() {
	channelsui_init();

	if (channels_->isVisible()) {
		channels_->raise();
		channels_->activateWindow();
	}
	else {
		channels_->show();
	}
